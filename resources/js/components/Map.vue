  ...
  methods: {
    setupLeafletMap: function () {
      ...
      for (const [providerName, providerData] of Object.entries(
        configLeaflet.tileLayerProviders
      )) {
        baseLayers[providerName] = L.tileLayer(
          providerData.urlTemplate,
    ...
    async getGroupedPlaces() {
      ...
      if (searchPID && searchGroup) {
        ...
        const place = ((this.groupedPlaces[searchGroup] || {}).places || {})[
          searchID
        ];
        place &&
          setTimeout(() => {
    ...
    getPersons() {
      this.axios
        .get("/api/wikidata/persons/cache")
        .then((response) => {
    ...
    updateInfoSidebarData() {
      ...
      if (personGroups.includes(this.selectedPlace.groupName)) {
        ...
        let location =
          this.groupedPlaces[this.selectedPlace.groupName]["places"][
            this.selectedPlace.id
          ];
        this.setSelectedPlace(
          location,
          this.selectedPlace.latLng,
    ...
    applyFilters(minDate, maxDate, groupNames) {
      ...
      Array.from(this.markersDate.entries()).forEach((entry) => {
        const groupName = entry[1].groupName;
        const startDate = new Date(entry[1].startDate);
        const endDate = new Date(entry[1].endDate);
        const applies =
          (endDate.getFullYear() > this.minDate.year ||
            (endDate.getFullYear() === this.minDate.year &&
              endDate.getMonth() >= this.minDate.month)) &&
          (startDate.getFullYear() < this.maxDate.year ||
            (startDate.getFullYear() === this.maxDate.year &&
              startDate.getMonth() <= this.maxDate.month)) &&
          (!this.groupNames || this.groupNames.includes(groupName));

        applies ? entry[0].addTo(this.map) : entry[0].remove();
      });
    },
    ...
    createPlaceMarkers: function (placeGroupName, places) {
      let placeMarkers = [];
      let iconUrl =
        this.$ogtGlobals.proxyPath +
        this.mapMarkerIconsPath +
        this.groupedPlaces[placeGroupName].iconName;
      const defaultIcon = L.icon({
        ...
        shadowUrl:
          this.$ogtGlobals.proxyPath + "/images/leaflet/marker-shadow.png",
      ...
      for (const [placeId, place] of Object.entries(places)) {
        ...
        for (const [statementId, placeCoordinate] of Object.entries(
          place.coordinates
        )) {
          ...
          this.locationMarkers[placeId][
            Object.values(placeCoordinate.value).join(",")
          ] = marker;
          const getDateProp = (key) => {
            return (
              place[key]?.value || Object.values(place[key] ?? {})[0]?.value
            );
          };
          this.markersDate.set(marker, {
            startDate:
              getDateProp("inceptionDates") ??
              getDateProp("startTime") ??
              (placeGroupName === "memorials"
                ? new Date("1945-05-08").toDateString()
                : new Date("1933-01-30").toDateString()),
            endDate:
    ...
    setSelectedPlace: function (place, latLng, placeGroupName) {
      ...
      this.selectedPlace.layerName =
        this.groupedPlaces[placeGroupName].layerName;
      ...
      window.history.replaceState(
        ...
        null,
        encodeURI(
          `${document.location.pathname}?id=${place.id}&group=${placeGroupName}&lat=${latLng.lat}&lng=${latLng.lng}`
        )
      );
      ...
      const coordinatesKeys = place.coordinates
        ? Object.keys(place.coordinates)
        : [];
      ...
      if (imagesCounted == 1 && coordinatesCounted == 1) {
        ...
        this.selectedPlace.mainImageLegend = place.images[imageKeys[0]]
          .mediaLegend
          ? place.images[imageKeys[0]].mediaLegend.value
          : "";
        ...
        imageKeys.some((imageKey) => {
          ...
          if (
            ...
            this.selectedPlace.mainImageLegend = image.mediaLegend
              ? image.mediaLegend.value
              : "";
            return true;
          }
        });
      ...
      if (place.inceptionDates) {
        for (const [statementId, inceptionDate] of Object.entries(
          place.inceptionDates
        )) {
          this.selectedPlace.inceptionDate = this.getDate(
            inceptionDate.value,
            inceptionDate.datePrecision
          );
          break;
        }
      }
      ...
      if (place.dissolvedDates) {
        for (const [statementId, dissolvedDate] of Object.entries(
          place.dissolvedDates
        )) {
          this.selectedPlace.dissolvedDate = this.getDate(
            dissolvedDate.value,
            dissolvedDate.datePrecision
          );
          break;
        }
      }
      ...
      for (const [statementId, coordinate] of Object.entries(
        place.coordinates
      )) {
        let label = coordinate.streetAddress
          ? coordinate.streetAddress.value
          : "";

        let startDate = coordinate.startTime
          ? this.getDate(
              coordinate.startTime.value,
              coordinate.startTime.datePrecision
            )
          : null;

        let endDate = coordinate.endTime
          ? this.getDate(
              coordinate.endTime.value,
              coordinate.endTime.datePrecision
            )
          : null;
        ...
        if (
          coordinate.sourcingCircumstance &&
          coordinate.sourcingCircumstance.id == "Q13649246"
        ) {
          isUncertain = true;
        }
        ...
        if (
          coordinate.value.lat == latLng.lat ||
          coordinate.value.lng == latLng.lng
        ) {
      ...
      if (place.streetAddresses) {
        ...
        if (streetAddressKeys.length == 1 && additionalAddresses.length == 0) {
          ...
          if (address.startTime) {
            selectedAddress.startDate = this.getDate(
              address.startTime.value,
              address.startTime.datePrecision
            );
          }

          if (address.endTime) {
            selectedAddress.endDate = this.getDate(
              address.endTime.value,
              address.endTime.datePrecision
            );
          }
        } else {
          streetAddressKeys.forEach((addressKey) => {
            ...
            let startDate = streetAddress.startTime
              ? this.getDate(
                  streetAddress.startTime.value,
                  streetAddress.startTime.datePrecision
                )
              : null;

            let endDate = streetAddress.endTime
              ? this.getDate(
                  streetAddress.endTime.value,
                  streetAddress.endTime.datePrecision
                )
              : null;
      ...
      if (place.employeeCounts) {
        for (const [statementId, employeeCount] of Object.entries(
          place.employeeCounts
        )) {
          let pointInTime = employeeCount.pointInTime
            ? this.getDate(
                employeeCount.pointInTime.value,
                employeeCount.pointInTime.datePrecision
              )
            : null;
          ...
          if (
            ...
            employeeCount.sourcingCircumstance.id in this.sourceCircumstances
          ) {
            sourcingCircumstance =
              this.sourceCircumstances[employeeCount.sourcingCircumstance.id];
          }
      ...
      this.selectedPlace.directors = this.getPropertyData(
        place.directors,
        true,
        false,
        true
      ...
      if (place.prisonerCounts) {
        for (const [statementId, prisonerCount] of Object.entries(
          place.prisonerCounts
        )) {
          let sourcingCircumstance = "";
          if (
            prisonerCount.sourcingCircumstance &&
            prisonerCount.sourcingCircumstance.id in this.sourceCircumstances
          ) {
            sourcingCircumstance =
              this.sourceCircumstances[prisonerCount.sourcingCircumstance.id];
          }
      ...
      this.selectedPlace.isDeportationEvent = this.hasItemPropertyClass(
        place,
        "instances",
        "deportation"
      );
      ...
      if (place.significantEvents) {
        for (const [statementId, significantEvent] of Object.entries(
          place.significantEvents
        )) {
          let hasLocationMarker = this.locationMarkers[significantEvent.id]
            ? true
            : false;
      ...
      if (place.parentOrganizations) {
        for (const [statementId, parentOrganization] of Object.entries(
          place.parentOrganizations
        )) {
          let hasLocationMarker = this.locationMarkers[parentOrganization.id]
            ? true
            : false;

          let startDate = parentOrganization.startTime
            ? this.getDate(
                parentOrganization.startTime.value,
                parentOrganization.startTime.datePrecision
              )
            : null;

          let endDate = parentOrganization.endTime
            ? this.getDate(
                parentOrganization.endTime.value,
                parentOrganization.endTime.datePrecision
              )
            : null;
      ...
      if (place.subsidiaries) {
        for (const [statementId, subsidiary] of Object.entries(
          place.subsidiaries
        )) {
          let hasLocationMarker = this.locationMarkers[subsidiary.id]
            ? true
            : false;

          let startDate = subsidiary.startTime
            ? this.getDate(
                subsidiary.startTime.value,
                subsidiary.startTime.datePrecision
              )
            : null;

          let endDate = subsidiary.endTime
            ? this.getDate(
                subsidiary.endTime.value,
                subsidiary.endTime.datePrecision
              )
            : null;
      ...
      if (place.replaces) {
        for (const [statementId, replace] of Object.entries(place.replaces)) {
          let hasLocationMarker = this.locationMarkers[replace.id]
            ? true
            : false;

          let pointInTime = replace.pointInTime
            ? this.getDate(
                replace.pointInTime.value,
                replace.pointInTime.datePrecision
              )
            : null;
      ...
      if (place.replacedBys) {
        for (const [statementId, replacedBy] of Object.entries(
          place.replacedBys
        )) {
          let hasLocationMarker = this.locationMarkers[replacedBy.id]
            ? true
            : false;

          let pointInTime = replacedBy.pointInTime
            ? this.getDate(
                replacedBy.pointInTime.value,
                replacedBy.pointInTime.datePrecision
              )
            : null;
      ...
      if (place.describedBySources) {
        for (const [statementId, describedBySource] of Object.entries(
          place.describedBySources
        )) {
      ...
      if (place.officialWebsite) {
        for (const [statementId, website] of Object.entries(
      ...
      if (place.hasUses) {
        for (const [statementId, hasUse] of Object.entries(place.hasUses)) {
          const value =
            hasUse.value === "Ausbildung" ? "Bildungsarbeit" : hasUse.value;
      ...
      if (place.startTime) {
        for (const [statementId, startDate] of Object.entries(
          place.startTime
        )) {
          this.selectedPlace.startDate = this.getDate(
            startDate.value,
      ...
      if (place.openingDate) {
        for (const [statementId, openingDate] of Object.entries(
          place.openingDate
        )) {
          this.selectedPlace.openingDate = this.getDate(
            openingDate.value,
            openingDate.datePrecision
          );
          break;
        }
      }
      ...
      if (place.endTime) {
        for (const [statementId, endTime] of Object.entries(place.endTime)) {
          this.selectedPlace.endDate = this.getDate(
            endTime.value,
            endTime.datePrecision
          );
          break;
        }
      }
      ...
      if (place.pointInTime) {
        for (const [statementId, pointInTime] of Object.entries(
          place.pointInTime
        )) {
          this.selectedPlace.pointInTime = this.getDate(
            pointInTime.value,
            pointInTime.datePrecision
          );
          break;
        }
      }
      ...
      if (place.commemorates) {
        for (const [statementId, commemorate] of Object.entries(
          place.commemorates
        )) {
          let hasLocationMarker = this.locationMarkers[commemorate.id]
            ? true
            : false;
      ...
      if (place.perpetrators) {
        for (const [statementId, perpetrator] of Object.entries(
          place.perpetrators
        )) {
          let hasPersonData = this.persons.perpetrators[perpetrator.id]
            ? true
            : false;
          let hasLocationMarker = this.locationMarkers[perpetrator.id]
            ? true
            : false;
      ...
      if (place.numberOfCasualties) {
        for (const [statementId, numberOfCasualties] of Object.entries(
          place.numberOfCasualties
        )) {
      ...
      if (place.numberOfDeaths) {
        for (const [statementId, numberOfDeaths] of Object.entries(
          place.numberOfDeaths
        )) {
      ...
      if (place.numberOfSurvivors) {
        for (const [statementId, numberOfSurvivors] of Object.entries(
          place.numberOfSurvivors
        )) {
      ...
      if (place.numberOfArrests) {
        for (const [statementId, numberOfArrests] of Object.entries(
          place.numberOfArrests
        )) {
      ...
      if (place.significantPlaces) {
        for (const [statementId, significantPlace] of Object.entries(
          place.significantPlaces
        )) {
      ...
      if (place.startPoints) {
        for (const [statementId, startPoint] of Object.entries(
          place.startPoints
        )) {
      ...
      if (place.destinationPoints) {
        for (const [statementId, destinationPoint] of Object.entries(
          place.destinationPoints
        )) {
      ...
      if (derivedPlacesData.commemoratedBy) {
        ...
        for (const [statementId, commemoratedBy] of Object.entries(
          this.selectedPlace.commemoratedBy
        )) {
          commemoratedBy.hasLocationMarker = this.locationMarkers[
            commemoratedBy.id
          ]
            ? true
            : false;
      ...
      let directorIds = this.selectedPlace.directors.map(
        (director) => director.id
      );
      // only employees who are not directors of a location
      this.selectedPlace.employees = this.getPropertyData(
        derivedPlacesData.employees,
        true,
        false,
        false,
        directorIds
      ...
      if (derivedPlacesData.victims) {
        for (const [index, victim] of Object.entries(
          derivedPlacesData.victims
        )) {
    ...
    hasItemPropertyClass: function (item, propertyName, className) {
      ...
      if (item[propertyName]) {
        ...
        for (const [statementId, propertyValue] of Object.entries(
          item[propertyName]
        )) {
    ...
    sortByDate: function (itemA, itemB) {
      if (itemA.startDate && itemB.startDate) {
        return (
          itemA.startDate.value.getTime() - itemB.startDate.value.getTime()
        );
    ...
    sortByPointInTime: function (itemA, itemB) {
      if (itemA.pointInTime && itemB.pointInTime) {
        return (
          itemA.pointInTime.value.getTime() - itemB.pointInTime.value.getTime()
        );
      ...
      for (const [memorialId, memorial] of Object.entries(memorials)) {
        for (const [statementId, commemorate] of Object.entries(
          ...
          if (!this.derivedPlacesData.hasOwnProperty(commemorate.id)) {
            ...
            !this.derivedPlacesData[commemorate.id].hasOwnProperty(
              "commemoratedBy"
            )
          ) {
    ...
    deriveLocationEmployees: function () {
      ...
      for (const [perpetratorId, perpetrator] of Object.entries(perpetrators)) {
        if (perpetrator.employers) {
          for (const [statementId, employer] of Object.entries(
            perpetrator.employers
          )) {
            if (!this.derivedPlacesData.hasOwnProperty(employer.id)) {
              ...
              !this.derivedPlacesData[employer.id].hasOwnProperty("employees")
            ) {
    ...
    deriveDataFromVictims: function () {
      ...
      for (const [victimId, victim] of Object.entries(victims)) {
        if (victim.detentionPlaces) {
          for (const [statementId, detentionPlace] of Object.entries(
            victim.detentionPlaces
          )) {
            if (!this.derivedPlacesData.hasOwnProperty(detentionPlace.id)) {
              ...
              !this.derivedPlacesData[detentionPlace.id].hasOwnProperty(
                "prisoners"
              )
            ) {
        ...
        if (victim.significantEvents) {
          for (const [statementId, significantEvent] of Object.entries(
            ...
            if (!this.derivedPlacesData.hasOwnProperty(significantEvent.id)) {
              ...
              !this.derivedPlacesData[significantEvent.id].hasOwnProperty(
                "victims"
              )
            ) {
    ...
    checkUrlForLocation: function () {
      if (
        "qId" in this.$route.query &&
        this.$route.query.qId in this.locationMarkers
      ) {
    ...
    showPerson: function ({ id, group = "" }) {
      ...
      window.history.replaceState(
        ...
        null,
        encodeURI(
          `${document.location.pathname}?pid=${person.id}&group=${this.selectedPlace.groupName}`
        )
      );
    },
    /**
    ...
    setSelectedPerpetratorData: function (person) {
      ...
      this.selectedPlace.employers = this.getPropertyData(
        person.employers,
        true,
        true,
        false
    ...
    getPropertyData: function (
      itemProperty,
      addTimePeriods = false,
      addLinkedLocation = false,
      addLinkedPerson = false,
      excludedIds = []
    ) {
    ...
    getPropertyDataTimePeriods: function (propertyData) {
      ...
      if (propertyData.startTime) {
        startDate = this.getDate(
          propertyData.startTime.value,
          propertyData.startTime.datePrecision
        );
      } else if (propertyData.earliestDate) {
        startDate = this.getDate(
          propertyData.earliestDate.value,
          propertyData.earliestDate.datePrecision
        );
      }

      let maxStartDate = propertyData.latestStartDate
        ? this.getDate(
            propertyData.latestStartDate.value,
            propertyData.latestStartDate.datePrecision
          )
        : null;
      ...
      if (propertyData.endTime) {
        endDate = this.getDate(
          propertyData.endTime.value,
          propertyData.endTime.datePrecision
        );
      } else if (propertyData.latestDate) {
        endDate = this.getDate(
          propertyData.latestDate.value,
          propertyData.latestDate.datePrecision
        );
      }

      let minEndDate = propertyData.earliestEndDate
        ? this.getDate(
            propertyData.earliestEndDate.value,
            propertyData.earliestEndDate.datePrecision
          )
        : null;
    ...
    hasPersonData: function (personId) {
      return this.persons.victims[personId] ||
        this.persons.perpetrators[personId]
        ? true
        : false;
    },
    /**
    ...
    setSelectedVictimData: function (person) {
      ...
      if (person.detentionPlaces) {
        for (const [statementId, detention] of Object.entries(
          person.detentionPlaces
        )) {
          let hasLocationMarker = this.locationMarkers[detention.id]
            ? true
            : false;

          let startDate = detention.startTime
            ? this.getDate(
                detention.startTime.value,
                detention.startTime.datePrecision
              )
            : null;

          let endDate = detention.endTime
            ? this.getDate(
                detention.endTime.value,
                detention.endTime.datePrecision
              )
            : null;
    ...
    setSelectedPersonData: function (person) {
      ...
      if (person.givenName) {
        for (const [statementId, givenName] of Object.entries(
          person.givenName
        )) {
      ...
      if (person.familyName) {
        for (const [statementId, familyName] of Object.entries(
          person.familyName
        )) {
      ...
      if (person.placeOfBirth) {
        for (const [statementId, placeOfBirth] of Object.entries(
          person.placeOfBirth
        )) {
      ...
      if (person.placeOfDeath) {
        for (const [statementId, placeOfDeath] of Object.entries(
          person.placeOfDeath
        )) {
      ...
      if (person.dateOfBirth) {
        for (const [statementId, dateOfBirth] of Object.entries(
          person.dateOfBirth
        )) {
          this.selectedPlace.dateOfBirth = this.getDate(
            dateOfBirth.value,
      ...
      if (person.dateOfDeath) {
        for (const [statementId, dateOfDeath] of Object.entries(
          person.dateOfDeath
        )) {
          this.selectedPlace.dateOfDeath = this.getDate(
            dateOfDeath.value,
      ...
      if (person.citizenships) {
        for (const [statementId, citizenship] of Object.entries(
          person.citizenships
        )) {
